---
title: "Analysis of Possible Demorgraphic Factors Influencing Report of Mental Health Act Apprehensions in Toronto Area"
...
date: today
date-format: long
abstract: "To have a general idea in terms of statistics of Mental Health Act (MHA) Apprehensions in Toronto area, two of the dataset publically avaliable from Toronto Open Data Portal are speculated by using histograms and scatter plots. In general, the number of apprenhsions increases by year and a decrease in the number starts in 2022, males in age cohort 25-34 are more likely to have mental health related apprehensions, and the total number of the crimes and MHA apprehensions might also have indirect correlation within the same area. Although possible trends are detected, no relations are actually analyzed at this point of time. This report purposes to detect possible trends for future inspection and research."
format: pdf
toc: true
...
By statistics, in any given year, 1 in 5 Canadians experiences a mental illness. By the time Canadians reach 40 years old of age, 1 in 2 have - or have had - a mental illness [@CAMH]. Mental health issues are not uncommon and should be treated seriously just like other types of health issues. Toronto is one of the major cities in Canada, which is also the capital of Province Ontario. Toronto fields includes old 140 City of Toronto Neighborhoods structure as well as the new 158 City of Toronto Neighborhoods structure. The mental health of residents draw our attention of whether people are having healthy mental status at this big city. The Mental Health Act (MHA) apprehensions and reports hints the overall well-being of the residents in the area. This report is interested in possible factors that contributes to the numbers of apprehensions under MHA. 

This particular report thus examines the MHA Apprehensions data set [@torontoMHA] provided by Toronto Public Data by using R language [@citeR] to provide a overview of the statistics both demographic and year-related in regards of the mental health apprehensions. To elaborate, the relationships between number of reports and year, gender, and age are studied in this report.To hypothesize, the demographic factors gender and age correlates with the number of MHA Apprehensions. In addition, the number of Mental Health Apprehensions could be in some way related to total number of reported crimes in the same neighborhood. Thus, in addition to the Mental Health Apprehensions, Police Annual Statistical Report Data [@torontoPoliceAnnual] regarding arrested and charged persons from Open Data Toronto [@opendatatoronto] is loaded for comparing the number of crimes reported and number of MHA apprehensions in the same area to see if there is any correlation between the two. 

In result, potential trends in terms of relation between the demographic factors and crime rate in the same area division were detected. The relations were not statistically analyzed at this point of time, however, this report provides a foundation for research studies regarding the similar topics for Toronto area in the future. This report includes the introduction of the data sets used, demographic analysis, and a discussion with limitations at the end.


## Data {#sec-data}

For the report, data used are retrieved from Public online resource called Open Data Toronto Portal through the library `opendatatoronto` [@opendatatoronto].The data selected for the purpose of analysis are two package data sets called "Mental Health Act Apprehensions" [@torontoMHA] and "Police Annual Statistical Report - Arrested and Charged Persons" [@torontoPoliceAnnual]. We downloaded, cleaned and tidied the data set using the statistical programming language R [@citeR] including the `tidyverse` [@tidyverse], `janitor` [@janitor], `ggplot2` [@ggplot2], `dplyr` [@dplyr], `readr` [@readr], `tibble` [@tibble], `kableExtra` [@kableExtra], and `knitr` [@knitr]
